...
Open `http://localhost:5173`. Routes: `/` (public — premium marketing homepage: hero + booking widget, trust indicators, services, routes/fleet teasers, why-us, how-it-works, final CTA), `/services`, `/routes` (public — all active routes with a "Book This Route" CTA into the booking flow), `/fleet` (public — active vehicles), `/about`, `/faq` (public — accordion), `/contact` (public — form posting to `POST /api/public/contact`), `/privacy-policy`, `/terms-and-conditions`, `/cookie-policy` (public — honest placeholder pages, no invented legal text), `/booking` (public — anonymous multi-step booking form, ending in a "Pay Now" step; accepts an optional route/date/time/passenger prefill via router state from the homepage widget or a route's "Book This Route" link), `/booking/payment/{bookingReference}?token=...` (public — check/retry payment status from a saved link), `/booking/payment/success` / `/booking/payment/cancelled` (public — Stripe/fake-checkout redirect targets), `/login`, `/driver` (Driver-only), `/driver/availability` (current status + schedule), `/admin` (Administrator-only — dashboard statistics, upcoming trips, and a notification summary), `/admin/routes`, `/admin/vehicles`, `/admin/drivers`, `/admin/drivers/{id}` (details + read-only schedule), `/admin/bookings` (search/filter/sort/paginate, including by payment status), `/admin/bookings/{id}` (detail, edit, assign/reassign, cancel, resend confirmation email, payment history + refund), `/admin/reports` (includes a Payments metrics section), `/unauthorized`.
...
| `GET /api/public/vehicles` | none | Active vehicles only, minimal fields — never `registrationNumber`/`notes` — for the public Fleet page and homepage teaser |
| `GET /api/public/company` | none | Company contact info/hours from `CompanySettings` config, with obvious placeholder defaults — for the Footer, Contact page, and page `<title>`/meta description |
| `POST /api/public/contact` | none | `{ name, email, phone?, subject, message }` → persists a `ContactMessage` (status `Pending`) and returns immediately; a background worker sends it to `NotificationSettings:AdminEmail` (skipped gracefully, message stays `Pending`, if unconfigured). `400` on invalid email/format, too-short message, or HTML markup in any field. `[RequestSizeLimit(16*1024)]` |
...
| `GET /api/account/preferences` | any authenticated user | `{ languageCode }` for the caller — `null` if never saved (frontend then falls back to the browser's language) |
| `PUT /api/account/preferences` | any authenticated user | `{ languageCode }` → saves the caller's language preference. An unsupported/malformed code is silently normalized to `"en"` (never rejected); `null` explicitly clears the preference |
| `GET /api/admin/reports/*/export` | `Administrator` | Every CSV export endpoint additionally accepts `?lang=` (`en`\|`de`\|`fr`, default `en`) to localize column headers only — the exported data itself is never translated |
...
### Prompt 16 — Multilingual Support and Internationalization

Three languages are supported: English (default), German, French. (Italian was originally included but was removed by explicit instruction — see below.)

- **`react-i18next` was introduced** (no i18n library existed before) — all three languages' translation JSON (`frontend/src/locales/{en,de,fr}/*.json`) are bundled at build time rather than lazy-loaded per language: the combined payload is a few KB, far below where code-splitting would meaningfully help, and bundling avoids a network round trip (and a loading flash) every time a customer switches language mid-session.
- **Detection order: `localStorage` → browser `navigator` language → English**, never IP-based geolocation, per the spec's explicit instruction. `i18next-browser-languagedetector`'s `supportedLngs`/`nonExplicitSupportedLngs` options mean an unsupported browser language (e.g. `es-ES`, or `it-IT` now that Italian has been removed) falls straight through to English rather than partially matching.
- **`Booking.LanguageCode`** (backend, new column, `en`/`de`/`fr`, default `en`) is captured once at booking creation from the customer's active UI language — not re-derived from their browser later — because the customer may book in German and read the confirmation email afterward on a different device/browser. **`User.LanguageCode`** (backend, new nullable column) is the equivalent for authenticated Admin/Driver users, set via `GET`/`PUT /api/account/preferences`; `null` means "no preference saved yet, use the browser's language" — never required at account creation.
- **A single `SupportedLanguages` helper (`Domain/Common`)** is the one place that decides what happens to an invalid/missing language code: silently normalize to English, never reject the request and never store an arbitrary code. Both `Booking`'s constructor and `User.SetLanguage(...)` route through it.
- **Email templates were restructured from one flat file per event into `Templates/{languageCode}/{EventName}.html`** (11 events × 3 languages, plus a per-language `_Layout.html` for the footer) — `EmailTemplateRenderer.Render(templateName, languageCode, fields)` looks in the requested language's folder first, then **falls back to the English folder** if that specific file is missing there, so a translation that hasn't been written yet can never leave an email unsent (section 25). Customer emails render in `Booking.LanguageCode`; driver emails render in the assigned driver's `User.LanguageCode` (English if unset); the admin manual-assignment-required email is always English, since it goes to a raw configured address (`NotificationSettings:AdminEmail`) with no associated `User` to have a preference.
- **A small `StatusTranslations` helper (`Application/Common`)** turns `BookingStatus`/`RideStatus`/`PaymentStatus` into natural, localized wording for the one place the backend renders status into human-readable text — inside emails. This is deliberately *not* a general-purpose backend localization layer: **the API and domain model stay language-neutral everywhere else** (`BookingResponse.Status` is always the raw enum name, e.g. `"Confirmed"`, never a translated word) — translation of API-facing statuses happens entirely on the frontend via `StatusBadge`, matching section 57-59's explicit instruction.
- **CSV report exports accept `?lang=`** (`en`/`de`/`fr`, default English) and localize only the column headers (`ReportCsvLabels`) — the exported data itself (names, references, `CHF`, city names) is never translated, per section 44's explicit example.
- **Frontend**: `LanguageSelector` (shared component, EN|DE|FR text buttons — never flags, per section 3) is wired into the public nav/booking page, the Admin sidebar, and the Driver sidebar. Switching language never resets in-progress booking form state (verified live and by a dedicated test) since it only ever calls `i18n.changeLanguage(...)`, nothing else. For an authenticated user, a change also calls `PUT /api/account/preferences` (best-effort — the UI has already switched regardless of whether that call succeeds); on login, the user's saved preference (if any) overrides whatever language was active before.
- **Fully translated**: the public site (nav, hero, footer strip), the entire booking flow (all 4 steps, validation messages, the confirmation/payment step), the three payment status pages, `StatusBadge` (booking/ride/payment statuses), and the Admin/Driver sidebar navigation + logout.
- **Not yet translated** (English-only, safe fallback — see **Known Issues** below): most individual field labels and table headers inside the admin Bookings/Drivers/Vehicles/Routes/Reports pages and the driver Schedule/Trip-detail/Profile pages. The i18n architecture, namespaces (`admin`, `driver`, `reports`), and translation-key patterns needed to finish this are all in place — it's additional key coverage, not new plumbing.
- **New indexes/columns**: `Bookings.LanguageCode` (`varchar(2)`, `NOT NULL DEFAULT 'en'`), `Users.LanguageCode` (`varchar(2)`, nullable) — migration `AddLanguageSupport`.
- **Do-not-implement list honored**: no machine/AI translation at runtime, no RTL support, no new payment provider/auth system/booking logic/notification system/reporting logic.
- **Italian removed by explicit instruction after initial delivery**: Italian (`it`) was originally implemented alongside English/German/French, then removed — `SupportedLanguages.Codes`, `SUPPORTED_LANGUAGES`, `StatusTranslations`, `ReportCsvLabels`, `LanguageSelector`, and `i18n.ts` all now only declare `en`/`de`/`fr`; the `Templates/it/` email folder and `frontend/src/locales/it/` translation bundle were deleted outright rather than left dangling and unreferenced. Any booking/user row that was previously stored with `LanguageCode = "it"` (from before this change) still works correctly — `SupportedLanguages.Normalize` treats `"it"` as any other unsupported code now and falls back to English at every read site, exactly like `"es"` or any other code that was never supported.

### Testing Multilingual Support Locally

1. Switch languages via the `EN | DE | FR` selector in the public nav, the booking page, or the Admin/Driver sidebar — the page updates immediately, in place.
2. Create a booking after switching to German/French; the confirmation page and the `bookingReference` are unaffected, but `POST /api/public/bookings`'s `languageCode` field is captured on the booking. Watch the console (see **Testing Emails Locally**) for `[DEV EMAIL] To: ... | Subject: ...` — the subject and body will be in the language the booking was created in, e.g. `Ihre Limousinenbuchungsanfrage — LM-...` for German — regardless of what language the UI is showing by the time the background worker actually sends it.
3. As an Admin/Driver, `PUT /api/account/preferences` with `{ "languageCode": "fr" }`, then log out and back in — the login response's `user.languageCode` reflects the saved choice, and the frontend applies it automatically on login.
4. `GET /api/admin/reports/bookings/export?lang=de` (or `fr`) to see localized CSV column headers with unchanged data.

### Prompt 17 — Customer Website Redesign and Premium Public Experience

- **Pages added**: `HomePage` (rewritten — hero + `HeroBookingWidget`, trust indicators, services grid, routes/fleet teasers backed by real API data, why-us, how-it-works, final CTA), `ServicesPage`, `RoutesPage` (public — distinct from the existing `pages/admin/routes/RoutesPage`), `FleetPage`, `AboutPage`, `FAQPage`, `ContactPage`, `LegalPage` (shared component for `/privacy-policy`, `/terms-and-conditions`, `/cookie-policy`, each an honest placeholder — no invented legal text, per the explicit "do not invent content" instruction).
- **Components added**: `Header` (sticky, becomes visually more compact past 24px scroll via an `isScrolled` class toggle; desktop inline nav, hamburger-triggered mobile panel below ~900px, `LanguageSelector` and the primary "Book a Ride" CTA reachable in both states), `Footer` (nav/services/contact columns + bottom legal bar, company details from `useCompanyInfo`), `HeroBookingWidget` (compact From/To/Date/Time/Passengers — From options are the distinct real route departure locations, To is scoped to routes actually departing from the selected From), `MobileBookingCta` (sticky bottom "Book a Ride", CSS-hidden above 640px).
- **New public, `[AllowAnonymous]` backend endpoints**: `GET /api/public/vehicles` (active vehicles only, `PublicVehicleResponse` deliberately excludes `registrationNumber`/`notes` — covered by a dedicated test that serializes the response and asserts neither ever appears in the JSON), `GET /api/public/company` (from a new `CompanySettings` config section with obvious placeholder defaults — `"[Company address not yet configured]"`, `"+41 00 000 00 00"` — never a plausible-looking fake business fact), `POST /api/public/contact`.
- **Contact form uses a separate, purpose-built outbox, not the existing `Notification`/`NotificationOutboxWorker`**: `Notification.BookingId` is a non-nullable required FK to `Booking`, so a contact message (which has no booking) structurally cannot use it without weakening that entity's invariant. Instead, `ContactMessage` (new entity, `Pending`/`Sent`/`Failed`) + `ContactMessageOutboxProcessor` + `ContactMessageOutboxWorker` mirror the exact same architectural shape (persist-then-background-send, `POST /api/public/contact` never sends synchronously) as a deliberate, parallel design rather than touching well-tested Prompt 11 code. It reuses `NotificationSettings:AdminEmail` (no new config) and sends via the existing `IEmailSender`; if `AdminEmail` isn't configured, the worker skips gracefully and the message stays `Pending` rather than erroring (verified live — see below) — unlike the booking outbox, a contact-message send failure is marked `Failed` permanently rather than retried, since there's no customer-facing consequence to "wait longer."
- **Booking-flow integration, not duplication**: the homepage hero widget and each Routes-page "Book This Route" link hand off into the *existing* `/booking` flow via React Router `state` (`navigate("/booking", { state: { routeId, bookingDate, pickupTime, passengerCount } })` / `<Link state={{ routeId }}>`), consumed by a lazy `useState` initializer in `BookingPage` that only pre-fills fields actually provided — the customer still lands on step 1 to review/confirm, nothing is auto-submitted or skips validation. No booking logic was reimplemented.
- **`SERVICES` config array** (`frontend/src/config/services.ts`) is the one place the six marketing service categories (Airport Transfers, Business Transfers, City-to-City Transfers, Corporate Travel, Events & Special Occasions, Private Chauffeur Service) are defined, since they aren't backend-stored entities — explicitly out of scope for this prompt per its own "don't build a full CMS" note. Routes and fleet vehicles, by contrast, are always fetched live, never hard-coded.
- **`useCompanyInfo`/`usePageMeta` are new shared hooks**: `useCompanyInfo` module-level-caches the one `GET /api/public/company` result so every page that renders `Footer`/`ContactPage` in the same session only fetches it once; `usePageMeta` sets/restores `document.title` and the meta description per page without pulling in a routing/SEO library.
- **FAQ accordion uses native `<details>`/`<summary>`**, not a custom component — keyboard and screen-reader accessibility come from the HTML spec itself, with zero extra ARIA wiring.
- **No customer accounts anywhere on the new pages** — the Contact form and booking widget are both fully anonymous, consistent with every earlier prompt's explicit no-customer-login requirement.
- **Trust-indicator copy avoids unverifiable claims**: the spec's own illustrative example text ("AVAILABLE 24/7") was replaced with an honest, unfalsifiable phrasing ("Ready When You Are" / "Book at your convenience") rather than asserting a round-the-clock guarantee the app has no way to actually confirm — per the spec's own explicit prohibition on inventing content like "24/7 availability" without it being backed by real configuration.
- **Do-not-implement list honored**: no customer accounts/reviews/testimonials, no fabricated awards/years-in-business/24-7 claims, no live chat widget, no full CMS for the marketing content (the `SERVICES` array is the pragmatic middle ground), no new booking/payment/notification/reporting logic — only integration with what already exists.

### Testing the Public Website Redesign Locally

1. `npm run dev` and open `/` — the hero widget's From/To dropdowns are populated from `GET /api/public/routes`; submitting navigates into `/booking` with the matched route (if any) and entered date/time/passengers pre-filled, landing on step 1 for review.
2. Visit `/routes` and `/fleet` — both are backed live by `GET /api/public/routes` / `GET /api/public/vehicles`; confirm no vehicle card ever shows a registration/license-plate number.
3. Submit `/contact` with a valid message — verified live end-to-end: the `POST /api/public/contact` call returns `200`, a new row appears in the `ContactMessages` table with `Status = Pending`, and (with `NotificationSettings:AdminEmail` left unconfigured, as in this environment) the background worker logs `Skipped processing pending contact messages — NotificationSettings:AdminEmail is not configured.` and leaves the message `Pending` rather than crashing — exactly the documented graceful-degradation behavior. Also verified live: an invalid email returns `400` (`"Email format is invalid."`), and a subject/message containing `<`/`>` returns `400` (`"HTML markup is not allowed."`).
4. Resize the browser down toward 320px width — the header is expected to collapse to the hamburger menu and the sticky `MobileBookingCta` to appear at the bottom of every marketing page, with no page requiring horizontal scrolling. All new CSS (`index.css`) reuses the existing design tokens/breakpoints and was reviewed for this, but — unlike the other checks above — this was **not** exercised in an actual browser this session (no browser automation was available); treat it as needing a visual pass before shipping.
5. Tab through the Header's mobile menu and the FAQ accordion using only the keyboard — both should be reachable and operable without a mouse (native `<details>`/`<summary>` and standard focusable buttons/links, so this is expected by construction, but hasn't been manually verified in a real browser either).
...
- **Prompt 17's responsive/visual behavior was not exercised in an actual browser** — no browser automation was available this session, so the new public pages were verified via the automated test suite (170 frontend tests, including `Header`/`Footer`/`HeroBookingWidget`/every new page), a live backend/API check of the three new public endpoints and the contact form (including its validation and graceful-degradation paths), and a manual review of the new CSS against the existing design tokens/breakpoints — but not a real-device or real-browser pass at the spec's listed breakpoints (320–1920px), and not a screen-reader pass. This should be done before shipping.
- **Company details are placeholder values** (`CompanySettings` in `appsettings.json`): phone `+41 00 000 00 00`, email `info@example.com`, address `"[Company address not yet configured]"`, opening hours `"[Opening hours not yet configured]"` — deliberately obvious placeholders per the "do not invent business facts" instruction. The Contact/About/legal pages will show these verbatim until real company information is provided and configured.
- **The three legal pages (`/privacy-policy`, `/terms-and-conditions`, `/cookie-policy`) contain a placeholder notice only**, no actual legal text — writing real privacy/terms/cookie policy content requires legal input this prompt explicitly said not to invent.
...
- **Translation coverage is not yet 100% of every UI string** (Prompt 16): the public site, the full booking flow, payment status pages, navigation, and statuses are fully translated in all three languages; most individual field labels/table headers inside the Admin (Bookings/Drivers/Vehicles/Routes/Reports detail pages) and Driver (Schedule/Trip detail/Profile) screens still render in English regardless of the selected language. This is additional translation-key coverage on the existing architecture, not missing plumbing — every namespace (`admin`, `driver`, `reports`) and the fallback-to-English behavior are already in place for it.
