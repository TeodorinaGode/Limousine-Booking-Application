...
| `GET /api/admin/reports/*/export` | `Administrator` | Every CSV export endpoint additionally accepts `?lang=` (`en`\|`de`\|`fr`\|`it`, default `en`) to localize column headers only — the exported data itself is never translated |
...
1. Switch languages via the `EN | DE | FR | IT` selector in the public nav, the booking page, or the Admin/Driver sidebar — the page updates immediately, in place.
2. Create a booking after switching to German/French/Italian; the confirmation page and the `bookingReference` are unaffected, but `POST /api/public/bookings`'s `languageCode` field is captured on the booking. Watch the console (see **Testing Emails Locally**) for `[DEV EMAIL] To: ... | Subject: ...` — the subject and body will be in the language the booking was created in, e.g. `Ihre Limousinenbuchungsanfrage — LM-...` for German — regardless of what language the UI is showing by the time the background worker actually sends it.
...
4. `GET /api/admin/reports/bookings/export?lang=de` (or `it`/`fr`) to see localized CSV column headers with unchanged data.
