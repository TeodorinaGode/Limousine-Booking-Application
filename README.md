...
Open `http://localhost:5173`. Routes: `/` (public), `/booking` (public — anonymous multi-step booking form), `/login`, `/driver` (Driver-only), `/driver/availability` (current status + schedule), `/admin` (Administrator-only — now also shows dashboard statistics + upcoming trips), `/admin/routes`, `/admin/vehicles`, `/admin/drivers`, `/admin/drivers/{id}` (details + read-only schedule), `/admin/bookings` (search/filter/sort/paginate), `/admin/bookings/{id}` (detail, edit, assign/reassign, cancel), `/unauthorized`.
...
- **Notification event boundaries (spec section 47) are not separately implemented** — no event bus, no publisher interface. The relevant transitions (booking created, confirmed via assignment, cancelled) are already single, well-defined call sites (`PublicBookingService.CreateBookingAsync`, `AutomaticAssignmentService`/`AdminBookingService`'s assignment methods, `AdminBookingService.CancelAsync`), which is where a future notification hook would attach — adding an actual event-dispatch abstraction now, with nothing subscribing to it, would be speculative infrastructure.
...
- Email/SMS notifications, online payment, reports, driver ride-status management, the driver mobile app, live tracking, reviews, promo codes, and customer accounts are all deliberately unimplemented — scoped for later steps, per Prompt 10's explicit "do not implement" list.
